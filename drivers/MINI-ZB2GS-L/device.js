...
class MINIZB2GSLOnOffBoundCluster extends BoundCluster {
...
class SonoffMINIZB2GSL extends SonoffBase {
    ...
    async onNodeInit({ zclNode }) {
        ...
        for (const [capability, endpointId] of Object.entries(CHANNEL_ENDPOINTS)) {
            ...
            if (endpoint) {
                endpoint.bind(CLUSTER.ON_OFF.NAME, new MINIZB2GSLOnOffBoundCluster(this, endpointId));
    ...
    async onSettings({ oldSettings, newSettings, changedKeys }) {
        ...
        for (const [settingKey, endpointId] of [['switch_mode_l1', 1], ['switch_mode_l2', 2]]) {
            ...
            try {
                ...
                await writeAttributesVerbose(this, this.zclNode.endpoints[endpointId].clusters[SonoffCluster.NAME], {
                    switch_mode: value,
                });
            } catch (err) {
        ...
        if (Object.keys(sonoffData).length > 0) {
            try {
                ...
                await writeAttributesVerbose(this, this.zclNode.endpoints[1].clusters[SonoffCluster.NAME], sonoffData);
                this.log('Write OK');
            } catch (err) {
    ...
    async checkAttributes() {
        ...
        for (const [settingKey, endpointId] of [['switch_mode_l1', 1], ['switch_mode_l2', 2]]) {
            try {
                const result = await this.zclNode.endpoints[endpointId].clusters[SonoffCluster.NAME]
                    .readAttributes('switch_mode');
