...
const { debug, CLUSTER, BoundCluster } = require('zigbee-clusters');
...
// Sonoff scene buttons (e.g. SNZB-01M) can act as a remote: alongside their
// own custom button-press reports, they also send `groups`/`identify`/`onOff`
// commands (their "control other devices directly" feature) to every
// endpoint. None of our devices implement those clusters, so each one was
// logged as a noisy "binding_unavailable" error, repeatedly. Bind a silent
// no-op handler per cluster so they're absorbed instead of logged.
class SilentNoOpBoundCluster extends BoundCluster {
	// Override the default dispatch entirely: acknowledge (or simply ignore)
	// any command without needing every possible command pre-declared with
...
class SonoffBase extends ZigBeeDevice {
	...
	async onNodeInit({zclNode}, options) {
		...
		for (const endpoint of Object.values(zclNode.endpoints)) {
			for (const clusterName of ['groups', 'identify', 'onOff']) {
				if (!endpoint.bindings[clusterName]) {
					endpoint.bind(clusterName, new SilentNoOpBoundCluster());
				}
			}
		}
	...
	async writeAttributes(cluster, attribs, filter=null) {
		...
		cluster = cluster.NAME;
			...
			if (Object.keys(items).length === 0)
				return;
			await writeAttributesVerbose(this, clust, items)
				.catch(() => {
